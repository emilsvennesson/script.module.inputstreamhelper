...
class LinuxVendorTests(unittest.TestCase):
    ...
    def setUp(self):
        if not os.path.exists('tests/cdm/'):
            os.mkdir('tests/cdm/')
        with open('tests/cdm/libwidevinecdm_vendor.so', 'w') as fdesc:
            fdesc.write('Linux\n1.2.3.4')

    def tearDown(self):
        os.unlink('tests/cdm/libwidevinecdm_vendor.so')
...
class WindowsVendorTests(unittest.TestCase):
    ...
    def setUp(self):
        if not os.path.exists('tests/cdm/'):
            os.mkdir('tests/cdm/')
        with open('tests/cdm/widevinecdm_vendor.dll', 'w') as fdesc:
            fdesc.write('Windows\n1.2.3.4')

    def tearDown(self):
        os.unlink('tests/cdm/widevinecdm_vendor.dll')
...
class DarwinVendorTests(unittest.TestCase):
    ...
    def setUp(self):
        if not os.path.exists('tests/cdm/'):
            os.mkdir('tests/cdm/')
        with open('tests/cdm/libwidevinecdm_vendor.dylib', 'w') as fdesc:
            fdesc.write('macOS\n1.2.3.4')

    def tearDown(self):
        os.unlink('tests/cdm/libwidevinecdm_vendor.dylib')
